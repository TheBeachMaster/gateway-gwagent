...
@interface AJGWCGatewayCtrlGatewayMgmtApp ()

@property (nonatomic) ajn::gwcontroller::GatewayCtrlGatewayMgmtApp* handle;
@property (nonatomic) AJGWCGatewayCtrlSessionListenerAdapter* adapter;

@end

@implementation AJGWCGatewayCtrlGatewayMgmtApp
...
- (id)initWithHandle:(ajn::gwcontroller::GatewayCtrlGatewayMgmtApp*)handle
{
    self = [super init];
    if (self) {
        self.handle = (ajn::gwcontroller::GatewayCtrlGatewayMgmtApp *)handle;
    }
    return self;
}
...
- (id)initWithbusUniqueName:(NSString*) busUniqueName aboutData:(NSDictionary *) aboutData
{
    ...
    if (self) {
        ...
        self.handle = new ajn::gwcontroller::GatewayCtrlGatewayMgmtApp();
...
{
    std::vector<ajn::gwcontroller::GatewayCtrlConnectorApp*> connectorAppsVect;
    QStatus status = self.handle->retrieveConnectorApps(sessionId, connectorAppsVect);

    // Populate NSMutableArray with std::vector data
    for (std::vector<ajn::gwcontroller::GatewayCtrlConnectorApp*>::const_iterator vectIt = connectorAppsVect.begin(); vectIt != connectorAppsVect.end(); vectIt++) {
        [connectorApps addObject:[[AJGWCGatewayCtrlConnectorApp alloc] initWithHandle:*vectIt]];
    }
    return status;
}
...
{
    ajn::gwcontroller::GatewayCtrlSessionResult ret = self.handle->joinSession();
...
- (AJGWCGatewayCtrlSessionResult*)joinSession:(id<AJGWCGatewayCtrlSessionListener>) listener
{
    ...
    ajn::gwcontroller::GatewayCtrlSessionResult ret = self.handle->joinSession(self.adapter);
    return [[AJGWCGatewayCtrlSessionResult alloc]initWithStatus:ret.m_status sid:ret.m_sid];
}

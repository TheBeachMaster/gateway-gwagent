...
/**
 * The activity presents {@link AccessControlList} object and its {@link AccessRules}
 */
public class AclManagementActivity extends BaseActivity implements ListView.OnItemClickListener, 
                                                                   AclManagementHeaderFragment.AclNameListener,
                                                                   AclManagementButtonFragment.ActionButtonListener {
	...
	/**
	 * Configurable items adapter.
	 * The configurable items are located in the ItemsDrawer 
	 */
	...
	/**
	 * List of the drawer items.
	 * It will include the ExposedServices and the remoted apps
	 */
    ...
    static {
        ...
        try {
            ...
            createAclMethod                         = activClass.getDeclaredMethod("createAcl", 
            		                                                               String.class,
            		                                                               AccessRules.class);
            
            updateAclMethod                         = activClass.getDeclaredMethod("updateAcl", 
																                    String.class,
																                    AccessRules.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        if ( retrieveDataMethod == null || deleteAclMethod == null ||
        		createAclMethod == null || updateAclMethod == null) {
		...
		confItemsAdapter    = new AclMgmtConfigurableItemsAdapter( this,
				                                               R.layout.acl_management_drawer_item,
	...
	private void initDrawerListener() {
		...
		drawerToggle = new ActionBarDrawerToggle(
                ...
                R.string.acl_mgmt_drawer_open,
                R.string.acl_mgmt_drawer_close
                ) {
    ...
    /**
     * The ACL name has been changed. If it's not an empty String enable the actionButton 
     * on the {@link AclManagementButtonFragment}
	...
	/**
	 * Action button was clicked, handle the event according to the activity type 
	...
	private void retrieveData() {
        ...
        asyncTask = new AsyncTask<Void, Void, Void>() {
			...
			protected void onPostExecute(Void result) {
				...
				//If there are items in the drawable layout and no item was previously selected, then select the first item
	...
	private String retrieveRulesAsyncTask(int sid) {
		...
		if ( expServices.size() > 0 ) {
			...
			confItemsAdapter.add( new VisualAclConfigurableItem(this, expServices,
                                                                getString(R.string.acl_mgmt_exp_services_lbl)) );
		}
		...
		if ( remApps.size() > 0 ) {
			...
			for (RemotedApp remApp : remApps ) {
				...
				confItemsAdapter.add( new VisualAclConfigurableItem(this, remApp,
						                        remApp.getAppName() + "@" + remApp.getDeviceName()) );
			}
		}
	...
	private void createAcl(final String aclName, final AccessRules accessRules) {
        ...
        if ( sid == null ) {
            ...
            Log.d(TAG, "Can't create the ACL, no session with the GW is established, waiting for"
                         + " the onSessionJoined event");
	...
	private void updateAcl(final String aclName, final AccessRules accessRules) {
        ...
        if ( sid == null ) {
            ...
            Log.d(TAG, "Can't update the ACL, no session with the GW is established, waiting for"
                         + " the onSessionJoined event");
	 ...
	 * @param taskName Create or update ACL
	 * @param errMsg Error message on failure
	 * @param aclWriteResp {@link AclWriteResponse} 
	 */
	private void createUpdatePostExecAsyncTask(String taskName,String errMsg, AclWriteResponse aclWriteResp) {
		 ...
		 if ( invRules.getExposedServices().size() > 0 || invRules.getRemotedApps().size() > 0 ) {
			 ...
			 String msg = taskName + " succeeded. \n There are some rules not supported by the application which"
		               + " weren't sent to the gateway";
	 ...
	 * @param position Position of the selected item in the drawer
	 */
	...
	/**
	 * Show {@link AlertDialog} with verification whether to delete the ACL
	 * If the positive button was pressed, call the deleteAcl method 
	 */
	private void verifyDeleteRequest() {
	
		Builder builder = createAlertDialog("Delete ACL", "Are you sure you want delete the ACL: '" + 
		                                                         app.getSelectedAcl().getName() + "'");
	...
	private void deleteAcl() {
        ...
        if ( sid == null ) {
             ...
             Log.d(TAG, "Can't delete the ACL, no session with the GW is established, waiting for"
                        + " the onSessionJoined event");
        ...
        asyncTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPostExecute(Void result) {
                ...
                if ( gce != null || respCode != AclResponseCode.GW_ACL_RC_SUCCESS ) {
                	...
                	Log.w(TAG, "Failed to delete the ACL: '" + app.getSelectedAcl().getObjectPath() + "', " +
                	            "ACL response code: '" + respCode + "'");

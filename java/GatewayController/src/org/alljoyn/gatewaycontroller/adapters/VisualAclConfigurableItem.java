...
/**
 * {@link AccessControlList} configurable item.
 * Either "Exposed Services" or {@link RemotedApp}
 */
public class VisualAclConfigurableItem implements VisualItem {
	...
	/**
	 * Constructor
	 * @param activityContext The activity {@link Context}
	 * @param confItem The configuration item. {@link RemotedApp}
	 * or {@link List} of {@link ManifestObjectDescription} of the "Exposed Services"
	...
	/**
	 * @return TRUE whether this {@link VisualAclConfigurableItem} is a {@link RemotedApp}.
	 * Otherwise returns FALSE, which means that the {@link VisualAclConfigurableItem} is 
	 * the exposed service.
	 */
	...
	/**
	 * @return The configuration item. {@link RemotedApp}
	 * or {@link List} of {@link ManifestObjectDescription} of the "Exposed Services"
	 */
	 ...
	 * @param allowAllChecked if checked than we relate to all the rules as checked
	...
	/**
	 * Creates Map of the configured rules from the received list of {@link ManifestObjectDescription}
	 * @param rules List of {@link ManifestObjectDescription}
	 * @param interfaceNames The list of interfaces to be filled by this method with the names of the interfaces
	 * @return Map from {@link VisualInterface} to a {@link List} of {@link VisualObjectPath}
	 */
	private Map<VisualInterface, List<VisualObjectPath>> convertToVisualRules(List<ManifestObjectDescription> rules) {
		...
		//The value of this map will be incremented by one each time when a configured object path will be stored in the
		//set of the resRules. So if at the end the Integer value will be equal to the size of resRules.Set, it means that the 
		...
		for ( ManifestObjectDescription rule : rules ) {
			...
			for ( TPInterface iface : rule.getInterfaces() ) {
				...
				//Increase indicator counter only if the rule is configured, which means that the inserted 
		...
		for (VisualInterface vIface : resRules.keySet() ) {
			...
			//The value was incremented by one for each inserted configured object path 
	...
	/**
	 * Create list of {@link ManifestObjectDescription} from the given {@link Map} of rules.
	 * @param visualRules Map from {@link VisualInterface} to a {@link List} of {@link VisualObjectPath}
	 * @param allowAllChecked if TRUE, then all the rules will be considered as configured (checked)
	...
	/**
	 * @return Extracts rules to be configured in depends on the {@link VisualAclConfigurableItem#isRemotedApp()}
	 */

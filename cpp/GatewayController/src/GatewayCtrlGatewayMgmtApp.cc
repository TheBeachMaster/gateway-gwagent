...
using namespace ajn::gwcontroller::gwcConsts;

namespace ajn {
namespace gwcontroller {


QStatus GatewayCtrlGatewayMgmtApp::init(const qcc::String& gwBusName, ajn::services::AboutClient::AboutData const& aboutData)
{
    m_SessionHandler  = this;
    m_Listener = 0;


    return GatewayCtrlAnnouncedApp::init(gwBusName, aboutData);
}

GatewayCtrlGatewayMgmtApp::~GatewayCtrlGatewayMgmtApp()
{

}

void GatewayCtrlGatewayMgmtApp::emptyVector()
{
...
QStatus GatewayCtrlGatewayMgmtApp::retrieveConnectorApps(SessionId sessionId, std::vector<GatewayCtrlConnectorApp*>& connectorApps)
{
    QStatus status;
    {
        ...
        for (int i = 0; i < numApplications; i++) {
            ...
            GatewayCtrlConnectorApp*connectorApp = new GatewayCtrlConnectorApp();
...
GatewayCtrlSessionResult GatewayCtrlGatewayMgmtApp::joinSession() {
    return joinSession(NULL);
}


GatewayCtrlSessionResult GatewayCtrlGatewayMgmtApp::joinSession(GatewayCtrlSessionListener*listener) {
...
QStatus GatewayCtrlGatewayMgmtApp::joinSessionAsync(GatewayCtrlSessionListener*listener)
{
    m_Listener = listener;
...
QStatus GatewayCtrlGatewayMgmtApp::leaveSession()
{
    BusAttachment* busAttachment = GatewayCtrlGatewayMgmtAppController::getInstance()->getBusAttachment();
...
GatewayCtrlSessionListener* GatewayCtrlGatewayMgmtApp::getListener() const
{
    return m_Listener;
}


QStatus GatewayCtrlGatewayMgmtApp::release()
{
    emptyVector();

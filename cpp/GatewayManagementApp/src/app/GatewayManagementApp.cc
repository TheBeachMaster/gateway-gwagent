...
GatewayManagement* gatewayManagement = NULL;
...
{
    bus = new BusAttachment("GatewayManagementApp", true);
...
QStatus fillPropertyStore()
{
    ...
    status = propertyStoreImpl->setAppName("AllJoyn Gateway Agent Configuration Manager");
    if (status != ER_OK) {
        return status;
    }
...
void cleanup()
{
    if (gatewayManagement) {
        gatewayManagement->shutdownGatewayManagement();
        gatewayManagement = NULL;
    }
    if (bus) {
...
void signal_callback_handler(int32_t signum)
{
    if (signum == SIGCHLD) {
        ...
        GatewayManagement::sigChildCallback(signum);
    } else {
        s_interrupt = true;
    }
}
...
start:
    ...
    // Initialize GatewayManagement object
    ...
    status = gatewayManagement->initGatewayManagement(bus);
    if (status != ER_OK) {

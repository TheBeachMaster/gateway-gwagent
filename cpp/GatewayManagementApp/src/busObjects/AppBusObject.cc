...
AppBusObject::AppBusObject(BusAttachment* bus, GatewayApp* app, String const& objectPath, QStatus* status) :
    BusObject(objectPath.c_str()), m_App(app), m_ObjectPath(objectPath), m_AppStatusChanged(NULL),
...
void AppBusObject::GetAppStatus(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    QStatus status;
    status = replyArg[indx++].Set(AJPARAM_UINT16.c_str(), m_App->getInstallStatus());
    if (status != ER_OK) {
        goto ReplyError;
    }

    status = replyArg[indx++].Set(AJPARAM_STR.c_str(), m_App->getInstallDescription().c_str());
    if (status != ER_OK) {
        goto ReplyError;
    }

    status = replyArg[indx++].Set(AJPARAM_UINT16.c_str(), m_App->getConnectionStatus());
    if (status != ER_OK) {
        goto ReplyError;
    }

    status = replyArg[indx++].Set(AJPARAM_UINT16.c_str(), m_App->getOperationalStatus());
    if (status != ER_OK) {
        goto ReplyError;
    }
...
void AppBusObject::RestartApp(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    uint16_t responseCode = m_App->restartApp();
...
void AppBusObject::GetManifestFile(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    qcc::String manifestData = m_App->getManifest().getManifestData();
    ajn::MsgArg replyArg[1];
...
void AppBusObject::GetManifestInterfaces(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    QStatus status = marshalObjectDesciptions(m_App->getManifest().getExposedServices(), &replyArg[0]);
    if (status != ER_OK) {
    ...
    status = marshalObjectDesciptions(m_App->getManifest().getRemotedServices(), &replyArg[1]);
    if (status != ER_OK) {
...
QStatus AppBusObject::marshalObjectDesciptions(const GatewayAppManifest::ManifestPermissions& permissionsMap, MsgArg* msgArg)
{
    QStatus status = ER_OK;
    GatewayAppManifest::ManifestPermissions::const_iterator it;
    MsgArg* objectsArray = new MsgArg[permissionsMap.size()];
    size_t objectsIndex = 0;

    for (it = permissionsMap.begin(); it != permissionsMap.end(); it++) {

        const std::vector<GatewayManifestObject::InterfaceDesc>& interfaces = it->getInterfaces();
...
QStatus AppBusObject::SendAppStatusChangedSignal()
{
    ...
    GatewayBusListener* busListener = GatewayManagement::getInstance()->getBusListener();
    ...
    status = msgArg[indx++].Set(AJPARAM_UINT16.c_str(), m_App->getInstallStatus());
    if (status != ER_OK) {
        return status;
    }

    status = msgArg[indx++].Set(AJPARAM_STR.c_str(), m_App->getInstallDescription().c_str());
    if (status != ER_OK) {
        return status;
    }

    status = msgArg[indx++].Set(AJPARAM_UINT16.c_str(), m_App->getConnectionStatus());
    if (status != ER_OK) {
        return status;
    }

    status = msgArg[indx++].Set(AJPARAM_UINT16.c_str(), m_App->getOperationalStatus());
    if (status != ER_OK) {
        return status;
    }
...
void AppBusObject::GetMergedAcl(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    ajn::MsgArg replyArg[2];
    QStatus status = AclAdapter::marshalMergedAcl(m_App->getAcls(), replyArg);
    if (status != ER_OK) {
...
void AppBusObject::UpdateConnectionStatus(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    m_App->setConnectionStatus((ConnectionStatus)connectionStatus);
...
QStatus AppBusObject::SendAclUpdatedSignal()
{
    ...
    qcc::String destination = AJ_GW_APP_WKN_PREFIX + m_App->getAppId();
...
void AppBusObject::CreateAcl(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    QStatus status = AclAdapter::unmarshalAcl(msg, &aclName, &policy, &metaData, &customMetaData);
    if (status != ER_OK) {
    ...
    qcc::String aclId;
    uint16_t resultStatus = m_App->createAcl(&aclId, aclName, policy, metaData, customMetaData);
...
void AppBusObject::DeleteAcl(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    uint16_t responseCode = m_App->deleteAcl(aclId);
...
void AppBusObject::ListAcls(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    std::map<String, GatewayAcl*> acls = m_App->getAcls();

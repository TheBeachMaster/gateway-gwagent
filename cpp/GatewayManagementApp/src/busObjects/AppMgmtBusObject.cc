...
AppMgmtBusObject::AppMgmtBusObject(BusAttachment* bus, GatewayAppManager* gatewayManager, QStatus* status) :
    BusObject(AJ_GW_OBJECTPATH.c_str()), m_AppManager(gatewayManager)
{
...
void AppMgmtBusObject::GetInstalledApps(const InterfaceDescription::Member* member, Message& msg)
{
    ...
    std::map<String, GatewayApp*> apps = m_AppManager->getApps();
    std::vector<MsgArg> appInfo(apps.size());
    size_t appInfoSize = 0;
    for (it = apps.begin(); it != apps.end(); it++) {
        status = appInfo[appInfoSize++].Set(AJPARAM_INSTALLED_APPS_INFO.c_str(), it->second->getAppId().c_str(),
